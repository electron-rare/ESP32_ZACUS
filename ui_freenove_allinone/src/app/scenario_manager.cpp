...
bool ScenarioManager::gotoScene(const char* scene_id, uint32_t now_ms, const char* source) {
  ...
  for (uint8_t index = 0U; index < scenario_->stepCount; ++index) {
    ...
    const char* normalized_step_scene = storyNormalizeScreenSceneId(screen_scene_id);
    const bool direct_match = (std::strcmp(screen_scene_id, scene_id) == 0);
    const bool normalized_match = (normalized_scene != nullptr && normalized_step_scene != nullptr &&
                                   std::strcmp(normalized_step_scene, normalized_scene) == 0);
