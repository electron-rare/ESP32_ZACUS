...
struct SceneWs2812FxState {
  ...
  bool dominant_band_single = false;
...
struct SceneAmbientAudioFxState {
  ...
  uint32_t delay_max_ms = kUsonAmbientDelayMaxMs;
  bool eq_soft = true;
  bool eq_warm = true;
  bool eq_bright = true;
};
...
void buildHotlineCueSelector(const char* scene_id,
                             const char* validation_state,
                             char* out_cue,
                             size_t out_cue_capacity) {
  if (out_cue == nullptr || out_cue_capacity == 0U) {
    return;
  }
  const char* base_scene = (scene_id != nullptr && scene_id[0] != '\0') ? scene_id : "SCENE_UNKNOWN";
  const char* suffix = "indice_*";
  if (validation_state != nullptr) {
    if (std::strcmp(validation_state, "waiting") == 0) {
      suffix = "attente_validation*";
    } else if (std::strcmp(validation_state, "granted") == 0) {
      suffix = "validation_ok*";
    } else if (std::strcmp(validation_state, "refused") == 0) {
      suffix = "validation_ko*";
    }
  }
  std::snprintf(out_cue, out_cue_capacity, "%s/%s", base_scene, suffix);
  out_cue[out_cue_capacity - 1U] = '\0';
}
...
void syncHotlineSceneState(const ScenarioSnapshot& snapshot) {
  ...
  char cue_selector[72] = {0};
  buildHotlineCueSelector(snapshot.screen_scene_id, validation_state, cue_selector, sizeof(cue_selector));
  char payload[kHotlineSceneSyncPayloadCapacity] = {0};
  const int written = std::snprintf(payload,
                                    sizeof(payload),
                                    "SCENE {\"id\":\"%s\",\"step_id\":\"%s\",\"validation_state\":\"%s\",\"cue\":\"%s\"}",
  ...
  Serial.printf("[HOTLINE_SYNC] scene=%s step=%s validation=%s cue=%s ok=%u\n",
                snapshot.screen_scene_id,
                step_id,
                validation_state,
                cue_selector,
...
void applySceneResourcePolicy(const ScenarioSnapshot& snapshot) {
  ...
  const bool is_uson_scene =
      (screen_scene_id != nullptr && std::strcmp(screen_scene_id, "SCENE_U_SON_PROTO") == 0);
  const bool requires_mic =
      g_hardware_cfg.mic_enabled &&
      (is_la_scene || is_uson_scene || g_la_trigger.gate_active || g_la_trigger.timeout_pending);
...
void startLaTimeoutRecovery(const ScenarioSnapshot& snapshot, uint32_t now_ms) {
  ...
  if (g_hardware_started) {
    ...
    if (g_hardware_cfg.led_auto_from_scene) {
      g_hardware.setSceneHint("SCENE_U_SON_PROTO");
    }
  }
  LaTriggerService::resetState(&g_la_trigger, false);
  const bool routed = g_scenario.gotoScene("SCENE_U_SON_PROTO", now_ms, "la_timeout_recovery");
  Serial.printf("[LA_TRIGGER] timeout -> goto SCENE_U_SON_PROTO routed=%u\n", routed ? 1U : 0U);
...
void printRuntimeStatus() {
  ...
  Serial.printf("STATUS scenario=%s step=%s screen=%s pack=%s audio=%u track=%s codec=%s bitrate=%u profile=%u:%s fx=%u:%s vol=%u "
                ...
                "hw=%u mic=%u battery=%u cam=%u media_play=%u rec=%u res=%s pressure=%u mic_should=%u cam_allow=%u story_debug_bypass=%u render_lock=%s\n",
                scenario_id,
                step_id,
                screen_id,
                audio_pack,
                ...
                g_scenario.debugTransitionBypassEnabled() ? 1U : 0U,
                "lgfx_runtime_only");
}
...
void printUiSceneStatus() {
  ...
  document["render_backend_lock"] = "lgfx_runtime_only";
  ...
  document["overlay_draw_ok"] = ui.overlay_draw_ok;
  document["overlay_draw_fail"] = ui.overlay_draw_fail;
  document["overlay_startwrite_fail"] = ui.overlay_startwrite_fail;
  document["overlay_skip_busy"] = ui.overlay_skip_busy;
  ...
  document["story_debug_bypass"] = g_scenario.debugTransitionBypassEnabled();
...
bool executeStoryAction(const char* action_id, const ScenarioSnapshot& snapshot, uint32_t now_ms) {
  ...
  if (std::strcmp(action_id, "ACTION_QUEUE_SONAR") == 0) {
    ...
    if (snapshot.screen_scene_id != nullptr &&
      ...
      candidates[1] = "/music/SCENE_WIN.mp3";
      candidates[2] = "/music/SCENE_WIN_ETAPE.mp3";
...
void refreshSceneIfNeededImpl(bool force_render) {
  ...
  if (transition.scene_changed) {
    ...
    if (g_hardware_started) {
      g_hardware.clearManualLed();
      g_hardware.setSceneSingleRandomBlink(false, 0U, 0U, 0U, 0U, false);
    }
  }
...
void handleSerialCommandImpl(const char* command_line, uint32_t now_ms) {
  ...
  if (std::strcmp(command, "STORY_DEBUG_BYPASS") == 0) {
    if (argument == nullptr || argument[0] == '\0') {
      Serial.printf("ACK STORY_DEBUG_BYPASS enabled=%u\n", g_scenario.debugTransitionBypassEnabled() ? 1U : 0U);
      return;
    }
    bool enabled = false;
    if (!parseBoolToken(argument, &enabled)) {
      Serial.println("ERR STORY_DEBUG_BYPASS_ARG");
      return;
    }
    g_scenario.setDebugTransitionBypassEnabled(enabled, "serial");
    Serial.printf("ACK STORY_DEBUG_BYPASS enabled=%u\n", enabled ? 1U : 0U);
    return;
  }
...
void runRuntimeIteration(uint32_t now_ms) {
  ...
  startPendingAudioIfAny();
  updateUsonProtoAmbientAudio(g_scenario.snapshot(), now_ms);
