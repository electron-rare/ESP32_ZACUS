...
bool NetworkManager::enableEspNow() {
  ...
  last_espnow_recovery_ms_ = last_espnow_refresh_ms_;
...
bool NetworkManager::sendEspNowText(const uint8_t mac[6], const char* text) {
  ...
  esp_err_t err = esp_now_send(mac,
                               ...
                               payload_len);
  if (err == ESP_ERR_ESPNOW_NO_MEM || err == ESP_ERR_ESPNOW_FULL) {
  ...
  bool should_recover_once = (err == ESP_ERR_ESPNOW_NOT_INIT ||
                              err == ESP_ERR_ESPNOW_INTERNAL ||
                              err == ESP_ERR_ESPNOW_IF);
#ifdef ESP_ERR_ESPNOW_CHAN
  should_recover_once = should_recover_once || (err == ESP_ERR_ESPNOW_CHAN);
#endif
  if (should_recover_once) {
    const uint32_t now_ms = millis();
    if (last_espnow_recovery_ms_ == 0U || (now_ms - last_espnow_recovery_ms_) >= 1200U) {
      last_espnow_recovery_ms_ = now_ms;
      // WiFi mode/channel switches can desync ESP-NOW internals; recover with cooldown.
      disableEspNow();
      delay(4);
      if (enableEspNow()) {
...
void NetworkManager::refreshEspNowConnection(uint32_t now_ms) {
  ...
  if (err == ESP_OK) {
    if (espnow_refresh_fault_) {
      espnow_refresh_fault_ = false;
      Serial.println("[NET] ESP-NOW health restored");
    }
    return;
  }
  ...
  if (err == ESP_ERR_ESPNOW_NOT_INIT) {
    espnow_enabled_ = false;
    if (enableEspNow()) {
      Serial.println("[NET] ESP-NOW reinit from periodic health check");
      espnow_refresh_fault_ = false;
    } else {
      if (!espnow_refresh_fault_) {
        Serial.println("[NET] ESP-NOW health fault (reinit failed)");
      }
  ...
  if (!espnow_refresh_fault_) {
    Serial.printf("[NET] ESP-NOW periodic health err=%d\n", static_cast<int>(err));
  }
  espnow_refresh_fault_ = true;
}
